...
{
    char buf[64];
    snprintf(buf, sizeof(buf), "{\"type\": %d, \"i\":%u,\"t\": %d}", WebSocketMessageTypes::MotorTicksChange, index, ticks);
    return String(buf);
}
...
CustomCommand *move = new CustomCommand("move", [](String command)
    ...
    rightMotorSpeed = (int16_t)-constrain(CommandParser::GetCommandParameter(command, 2).toInt(), -PWMRANGE, PWMRANGE);
